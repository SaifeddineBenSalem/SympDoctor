...
def update_csv(data):
    file_path = os.path.join(os.path.dirname(__file__), 'Training2.csv')
    symptoms = data['symptoms']
    disease_name = symptoms['prognosis']
    all_symptoms = data['allSymptoms']
    del symptoms['prognosis']
    ...
    header = [word for word in header if word != 'prognosis']
    header.append('prognosis')

    # Prepare the new row with symptom values and the disease name
    new_row = [symptoms.get(symptom, 0) for symptom in header]  # Include symptoms values
    new_row.append(disease_name)  # Append the disease name
    ...
    for row in existing_data[1:]:
        updated_row = row[:-1]  # Exclude the disease name
        disease = row[-1]
        # Add default 0 for any new symptoms in the header
        updated_row += [0] * (len(header) - len(updated_row))
