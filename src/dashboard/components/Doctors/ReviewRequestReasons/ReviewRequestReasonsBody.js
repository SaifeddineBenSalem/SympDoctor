...
const ViewDiseasesListBody = () => {
  ...
  const fetchDiseases = () => {
    fetch('http://localhost:5000/api/diseases/getreasonsbyid', {
      method: 'POST',
      headers: {
  ...
  const filteredApplications = allDiseases.filter(application => {
    const reason = (application.comment || '').toLowerCase();
    const postingDate = (application.posting_date || '').toLowerCase();
    const id = application.id; // Assuming id is a direct property

    const searchLower = searchTerm.toLowerCase();
    return reason.includes(searchLower) || postingDate.includes(searchLower) || id.toString().includes(searchLower);
  });
      ...
      <div id="page-inner">
        ...
        <div className="row">
          <div className="col-md-12">
            ...
            <input
              type="text"
              placeholder="Search by reason, posting date, or ID"
              value={searchTerm}
              ...
              <tbody>
                {currentDiseases.map((disease, index) => (
                  <tr key={index} style={{ borderBottom: '1px solid #ddd' }}>
                    ...
                    <td style={{ padding: '10px', border: '1px solid #ddd' }}>{disease.comment}</td>
                    <td style={{ padding: '10px', border: '1px solid #ddd' }}>
                      {disease.posting_date}
                    </td>
                  </tr>
                ))}
              </tbody>
